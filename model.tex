...
\end{equation}
where $P(t_k|d_j)$ is the probability of topic $t_k$ occurring in image $j$. Formally we formulate image-level feature as $I$ by concatenating the appearance feature $d_j$ and topic distribution $P(w_i|d_j)$, and define the image-level label presence potential $\psi_{G}$ as follow:
\begin{equation}
